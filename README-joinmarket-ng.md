<p align="center">
  <img src="media/logo.svg" alt="JoinMarket NG Logo" width="200"/>
</p>

# JoinMarket-NG

JoinMarket-NG (Next Generation) is a modern Bitcoin CoinJoin implementation for improving transaction privacy. It enables users to collaborate in CoinJoin transactions to break the link between inputs and outputs, enhancing financial privacy on the Bitcoin network.

## Full Live Documentation

For full documentation, see [JoinMarket-NG Documentation](https://joinmarket-ng.github.io/joinmarket-ng/).

## Installation

See the [Installation Guide](https://joinmarket-ng.github.io/joinmarket-ng/install/) for detailed instructions.

### Main Modules

| Module | Description |
|--------|-------------|
| [jmcore](https://joinmarket-ng.github.io/joinmarket-ng/README-jmcore/) | Core functionality (cryptography, networking, protocol) |
| [jmwallet](https://joinmarket-ng.github.io/joinmarket-ng/README-jmwallet/) | Wallet management and Bitcoin operations |
| [taker](https://joinmarket-ng.github.io/joinmarket-ng/README-taker/) | CoinJoin transaction initiation |
| [maker](https://joinmarket-ng.github.io/joinmarket-ng/README-maker/) | Liquidity provision service |

### Extra Modules

| Module | Description |
|--------|-------------|
| [orderbook_watcher](https://joinmarket-ng.github.io/joinmarket-ng/README-orderbook-watcher/) | Orderbook monitoring |
| [directory_server](https://joinmarket-ng.github.io/joinmarket-ng/README-directory-server/) | Peer discovery service |
| [scripts](https://joinmarket-ng.github.io/joinmarket-ng/README-scripts/) | Utility scripts for development |

### Technical Documentation

[Technical Documentation](https://joinmarket-ng.github.io/joinmarket-ng/technical/) - Comprehensive documentation covering concepts, architecture, protocol, privacy mechanisms, wallet, configuration, security, development, and best practices.

## Live Orderbook

- [Live orderbook (mainnet)](https://joinmarket-ng.sgn.space/)
- [Live orderbook (signet)](https://joinmarket-ng-signet.sgn.space/)

## Community

- [Telegram - JoinMarket Community](https://t.me/joinmarketorg)
- [SimpleX - JoinMarket Community](https://smp12.simplex.im/g#bx_0bFdk7OnttE0jlytSd73jGjCcHy2qCrhmEzgWXTk)

## License

[MIT License](LICENSE)
