...
- **Swap input for taker CoinJoin privacy** (`--swap-input`): Optional feature that makes the taker's on-chain footprint indistinguishable from a maker's. Before building the CoinJoin transaction, the taker acquires an extra UTXO via a Lightning Network reverse submarine swap (LN -> on-chain). This swap input covers all maker fees, the transaction fee, and a fake "fee earned" amount, so the taker's change output appears to gain sats -- just like a maker's -- defeating subset sum analysis and fee-asymmetry heuristics. Uses the Electrum-compatible P2WSH HTLC protocol for non-interactive claim witness construction during CoinJoin signing. New CLI flags: `--swap-input/--no-swap-input`, `--swap-provider`. New config section: `[swap]`. Key sub-features:
  - **Nostr-first provider discovery**: Swap providers are discovered via Nostr kind 30315 offer events, with automatic PoW validation (`MIN_POW_BITS=5`). Provider `min_amount`, fee percentage, and mining fee are fetched before the CoinJoin confirmation prompt so the user sees full swap cost details upfront. Falls back to direct HTTP URL via `--swap-provider` or `provider_url` config.
  - **Nostr DM-based swap RPC**: All swap communication uses NIP-04 encrypted direct messages (kind 25582) over Nostr relays, eliminating any direct network connection to the provider. Requests are encrypted with ECDH + AES-256-CBC (`coincurve` + `cryptography`), signed with BIP-340 Schnorr, and correlated via `reply_to` field. Each session uses a fresh ephemeral keypair. Automatic failover across multiple relays with configurable timeout. New module: `taker/swap/nip04.py`.
  - **Prepay invoice support**: When the provider returns a `minerFeeInvoice` (prepay for mining fees), the taker pays both invoices concurrently -- the main hold invoice is fired as a background task while the prepay is awaited, since the Electrum swap server bundles them and neither settles until both HTLCs arrive.
  - **Trustless lockup detection**: The taker monitors for the lockup UTXO directly on-chain using its own `BlockchainBackend` rather than trusting the provider's status endpoint.
  - **Fee equalization**: When the swap amount is padded up to the provider's minimum (typically 20,000 sats), leftover sats are distributed across maker fees using a sequential leveling algorithm, making all maker fees more uniform. Controlled by `equalize_fees` config option (default: `true`).
  - **Tor stream isolation**: All swap-related Tor connections use the `SWAP` isolation category with unique SOCKS5 credentials, ensuring separate circuits from directory and peer traffic.
- **Lightning Network integration**: The taker connects to its own LND node (`lnd_rest_url`, `lnd_cert_path`, `lnd_macaroon_path` in `[swap]` config) to pay swap invoices automatically. Docker Compose adds `lnd-taker` and `lnd-setup` services (profile `e2e`/`all`) for regtest. New modules: `taker/swap/ln_client.py`, `scripts/setup-lightning.sh`.
- **Real Electrum swap server for e2e testing**: Docker Compose (profile `e2e`/`all`) now runs a real Electrum swap server instead of a mock HTTP provider. The Electrum daemon runs its `swapserver` plugin with a Lightning-enabled wallet, communicating exclusively via Nostr DMs (kind 25582) through the relay -- no HTTP ports are exposed. New services: `electrum-swap-server` (custom Dockerfile from spesmilo/electrum source, Python 3.13), `electrs` (getumbrel/electrs for Electrum protocol over Bitcoin Core), `nostr-relay` (scsibug/nostr-rs-relay). Cross-implementation Lightning channel: LND-taker opens a channel to Electrum's built-in LN node. Removed: `lnd-provider` and `swap-provider` (mock) services. E2E tests (`tests/e2e/test_swap_input_e2e.py`) fully rewritten for the Nostr-based architecture: provider discovery via kind 30315, encrypted swap creation via kind 25582 DMs, LN invoice payment, trustless lockup detection, P2WSH signing verification, and cross-compatibility checks against the Electrum server's HTLC construction.
- **Orderbook watcher swap providers page**: Added a new `/swap-providers` page that lists Electrum Lightning swap provider offers for both mainnet and signet. Discovery runs fully in the browser via direct WebSocket connections to Nostr relays (kind `30315`, d-tag `electrum-swapserver-5`), with no external CDN dependencies; all assets are served locally from the orderbook watcher static directory.
- **Swap providers page relay defaults**: Updated `/swap-providers` to use Electrum's default Nostr relay list out of the box for plug-and-play provider discovery.
...
- **Fake fee now sampled from orderbook distribution**: The taker's fake "fee earned" is now sampled from the actual fee distribution of non-selected makers in the current orderbook, rather than a static random range. Falls back to a derived range when fewer than 3 eligible offers are available. The `fake_fee_min`/`fake_fee_max` config fields have been removed.
...
- **Swap input: `confirmation_callback` crash**: The taker passed `swap_info=` to the CLI confirmation callback, but `confirmation_callback()` did not accept the parameter, causing a `TypeError` at runtime that aborted the CoinJoin before it started. Added `swap_info` parameter and display of swap cost details in the confirmation prompt.
- **Swap input: leftover sats leaked to taker change (privacy break)**: `equalize_maker_fees()` used floor division, leaving 0 to n-1 sats undistributed. These leftovers ended up in the taker's change output, creating a detectable fee asymmetry that could identify the taker via subset sum analysis. The remainder is now distributed by giving 1 extra sat to the first `r` makers (where `r` is the indivisible remainder), ensuring ALL leftover sats go to makers.
- **Swap input: fee equalization was optional (privacy break)**: The `equalize_fees` config toggle allowed disabling fee equalization entirely, causing all leftover sats to go to taker change. Fee equalization is now mandatory when swap input is active; the config field is deprecated and ignored.
- **Swap input: mining fee underestimated by ~97 vbytes**: `_estimate_tx_fee()` used a flat 68 vbytes/input for all inputs, but P2WSH swap inputs are ~165 vbytes due to the HTLC witness script. Added `num_swap_inputs` parameter for accurate P2WSH vsize calculation.
- **Swap input: tx_fee not recalculated after swap UTXO acquired**: The mining fee was calculated before the swap input was added, so the extra ~97 vbytes were not covered. The fee is now recalculated after swap acquisition, and `swap_needed` includes the additional mining cost for the P2WSH input itself.
- **Swap input: sequential invoice payment deadlocks with bundled invoices**: The Electrum swap server bundles the prepay (`minerFeeInvoice`) and main hold invoice via `bundle_payments()`, meaning neither settles until both HTLCs arrive. The original code paid them sequentially (prepay first, then main), but `SendPaymentSync` blocks until settlement, causing a deadlock. Fixed by firing the main hold invoice as a background `asyncio.Task` and paying the prepay concurrently; the prepay settles once both HTLCs arrive at the provider.
- **Swap input: Nostr DM RPC used provider's Docker-internal relay URLs**: `NostrSwapRPC` used `provider.relays` parsed from the Nostr offer event, which contained Docker-internal URLs (e.g., `ws://jm-nostr-relay:7000`) unreachable from the host. Fixed to use the client's own `nostr_relays` list, which are known-reachable since they were used for provider discovery.
- **Swap input: PoW nonce hex parsing fails on odd-length values**: The Electrum swap server sends `pow_nonce` values like `"0x0"`. After stripping `0x`, odd-length hex strings (e.g., `"0"`) caused `bytes.fromhex()` to fail. Fixed by zero-padding to even length before parsing.
- **E2e: Electrum swap server Docker infrastructure**: Multiple fixes to the Electrum swap server Docker setup: switched from incompatible `mempool/electrs:latest` to `getumbrel/electrs:v0.10.9` (romanz fork with `--cookie-file` auth); fixed Electrum config to use nested JSON dicts, `config_version: 3` (prevents SSL migration), `use_gossip: true` (enables non-trampoline LN routing), `lightning_listen` (incoming LN connections), stale shared artifact cleanup; added on-chain funding (1 BTC) for the Electrum wallet so reverse swaps have on-chain liquidity; fixed nostr-rs-relay authorization config (empty `pubkey_whitelist` blocks all writes); changed healthcheck to file-based check since `getinfo` RPC hangs with swapserver plugin loaded.
...
- **Duplicate `!sig` warnings from multi-directory makers**: When a maker is connected to multiple directory servers, each signature message arrives once per directory. Signatures are now deduplicated by value before verification, eliminating spurious warnings.
- **LND REST API compatibility**: Fixed `pay_invoice` to not send `timeout_seconds` (rejected with 400) and to send `fee_limit.fixed` as integer. Fixed `lookup_invoice` to pass `r_hash_str` as hex instead of base64.
